    ...
    <div class="box" style="margin-left: 240px;">
            <div class="container content_width">
                <div class="person_search">
                    <div class="title">教学大纲附件上传</div>
                </div>
                <div class="line"></div>
            </div>

            <div style="float: left;width: 200px;height: 400px">
                <form action="jiaoxuedagangUpload" method="post" enctype="multipart/form-data">
                    <p>上传教学大纲: <input type="file" name="fileName"/></p>
                    <p><input type="submit" value="提交"/></p>
                </form>
                        <#--判断是否上传文件-->
                <#if jiaoxuedagangmsg??>
                <span>${jiaoxuedagangmsg}</span><br>
                <#else >
                <span>${jiaoxuedagangmsg!("文件未上传")}</span><br>
                </#if>
                            <#--显示图片，一定要在img中的src发请求给controller，否则直接跳转是乱码-->
                <#if jiaoxuedagang??>
                <img src="/guanliyuan/show?fileName=${jiaoxuedagang}" style="width: 200px"/>
                <#else>
