    ...
    <div class="box" style="margin-left: 240px;">
            <div class="container content_width">
                <div class="person_search">
                    <div class="title">培训资料附件上传</div>
                </div>
                <div class="line"></div>
            </div>

            <div style="float: left;width: 200px;height: 400px">
                <form action="ziliaoUpload" method="post" enctype="multipart/form-data">
                    <p>上传培训资料: <input type="file" name="fileName"/></p>
                    <p><input type="submit" value="提交"/></p>
                </form>
                        <#--判断是否上传文件-->
                <#if ziliaomsg??>
                <span>${ziliaomsg}</span><br>
                <#else >
                <span>${ziliaomsg!("文件未上传")}</span><br>
                </#if>
                            <#--显示图片，一定要在img中的src发请求给controller，否则直接跳转是乱码-->
                <#if ziliao??>
                <img src="/guanliyuan/show?fileName=${ziliao}" style="width: 200px"/>
                <#else>
